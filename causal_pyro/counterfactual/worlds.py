...
from .index_set import IndexSet, indices_of, scatter, gather
...
def complement(world: IndexSet) -> IndexSet:
    ...
    return IndexSet(**{name: indices for name, indices in diff.items() if name in world})
...
class _LazyPlateMessenger(IndepMessenger):
    ...
    def _process_message(self, msg):
        if msg["type"] not in ("sample", "param") or pyro.poutine.util.site_is_subsample(msg):
            return
        if self.frame.name in IndexSet.join(indices_of(msg["value"]), indices_of(msg["fn"])):
    ...
    def _pyro_add_indices(self, msg):
        ...
        for name, indices in world.items():
            if name not in self.plates:
                ...
                assert 0 <= min(indices) <= len(indices) <= max(indices) <= self.plates[name].size, \
