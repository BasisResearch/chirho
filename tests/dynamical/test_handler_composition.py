...
vec_obs3 = NonInterruptingPointObservationArray(
                times=flight_landing_times,
                data=flight_landing_data)
...
class UnifiedFixtureDynamicsReparam(UnifiedFixtureDynamics):
    def observation(self, X: State[torch.Tensor]):
        ...
        u_ip = pyro.sample("u_ip", Normal(7., 2.).expand(X.I.shape[-1:]).to_event(1))
...
def test_smoke_inference_twincounterfactual_observation_intervention_commutes():
    ...
    assert pred['infected_passengers'].squeeze().shape == (num_samples, num_worlds, len(flight_landing_times))
