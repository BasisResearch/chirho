...
class BaseShortColumn(pyro.nn.PyroModule):
    ...
    @pyro.nn.PyroSample
    def width(self):  # z1
        return dist.Uniform(5.0, 15.0)

    @pyro.nn.PyroSample
    def depth(self):  # z2
        return dist.Uniform(15.0, 25.0)

    @pyro.nn.PyroSample
    def yield_stress(self):  # z3
        return dist.LogNormal(math.log(5.0), 0.5)

    @pyro.nn.PyroSample
    def bending_moment(self):  # z4
        return dist.Normal(2000.0, 400.0)

    @pyro.nn.PyroSample
    def axial_force(self):  # z5
        return dist.Normal(500.0, 100.0)
...
class HighFidelityShortColumn(BaseShortColumn):
    ...
    def _model(width, depth, yield_stress, bending_moment, axial_force):
        return (
            1
            - 4 * bending_moment / (width * depth**2 * yield_stress)
...
class LowFidelityShortColumn2(BaseShortColumn):
    ...
    def _model(width, depth, yield_stress, bending_moment, axial_force):
        return (
            1
            - bending_moment / (width * depth**2 * yield_stress)
            - (axial_force / (width * depth * yield_stress)) ** 2
        )
...
def test_shortcolumn_surrogates_correlation(surrogate):
    ...
    model = HighFidelityShortColumn(cost=1.0)
    ...
    logging.info(
        f"high fidelity range: {high_fidelity_samples.min(dim=-1)[0], high_fidelity_samples.max(dim=-1)[0]}"
    )

    assert not torch.any(torch.isnan(surrogate_samples))
    logging.info(f"surrogate mean: {surrogate_samples.mean(dim=-1)}")
    logging.info(
        f"surrogate range: {surrogate_samples.min(dim=-1)[0], surrogate_samples.max(dim=-1)[0]}"
    )

    corr = scipy.stats.pearsonr(
        high_fidelity_samples.detach().numpy(), surrogate_samples.detach().numpy()
    )
...
def test_multifidelity_importance_1_surrogates_mean(surrogate, num_biasing_samples):
    ...
    for run in range(num_runs):
        ...
        reparam = pyro.infer.reparam.strategies.AutoReparam(centered=0.0)
        ...
        surrogate_elbo = pyro.infer.Trace_ELBO(
            num_particles=100, vectorize_particles=True
        )(surrogate_, surrogate_guide)
        surrogate_elbo()  # initialize parameters

        # fit surrogate guide
        with warnings.catch_warnings(), pyro.validation_enabled(False):
            warnings.filterwarnings("ignore", category=torch.jit.TracerWarning)
            surrogate_elbo = torch.jit.trace_module(
                surrogate_elbo, {"forward": ()}, strict=False, check_trace=False
            )
        ...
        result = torch.mean(
            log_weights.exp() * model_trace.nodes["column"]["value"], dim=-1
        )
        logging.info(
            f"{result}, {torch.logsumexp(log_weights - log_weights.shape[-1], dim=-1).exp()}"
        )
    ...
    logging.info(
        f"estimated multifidelity mean: {torch.tensor(actual_results).mean().item()}"
    )
...
def test_multifidelity_importance_n_surrogates():
    high_fidelity = HighFidelityShortColumn(cost=1.0)
    ...
    mixture_weights = torch.tensor(
        [
            1 / surrogate.cost
            - pyro.infer.Trace_ELBO(num_particles=100)(high_fidelity, surrogate_guide)()
    ...
    (
        ...
        high_fidelity,
        multifidelity_biasing_distribution,
        num_samples=1000,
        max_plate_nesting=1,
    )
