...
def test_noop_point_interruptions(solver, model, init_state, start_time, end_time):
    with solver():
        observational_execution_result = simulate(model, init_state, start_time, end_time)
    ...
    with solver():
        with StaticInterruption(time=end_time / 4.0):  # roughly 1/4 of the way through the timespan
...
def test_noop_point_interventions(solver, model, init_state, start_time, end_time, intervene_state):
    """
        ...
        with solver():
            with StaticIntervention(
                ...
                with StaticIntervention(
                    ...
                    result_double_pi1 = simulate(model, init_state, start_time, end_time)
        ...
        with solver():
            with StaticIntervention(
                ...
                with StaticIntervention(
                    ...
                    result_double_pi2 = simulate(model, init_state, start_time, end_time)
...
def test_noop_dynamic_interruption(solver, model, init_state, start_time, end_time, event_fn_builder):
    observational_execution_result = solver()(simulate)(model, init_state, start_time, end_time)
