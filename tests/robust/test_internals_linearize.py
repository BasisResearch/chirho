...
def test_empirical_fisher_vp_against_analytical(loc: torch.Tensor, cov_mat: torch.Tensor, v: torch.Tensor):
    ...
    empirical_fisher_vp_func = make_empirical_fisher_vp(func_log_prob, log_prob_params, data, cov_mat=cov_mat)
...
def test_fisher_grad_smoke(data_config):
    ...
    assert torch.func.grad(f)(v).sum() != 0, "Zero gradients but expected non-zero gradients"

    # Check using autograd
    assert torch.autograd.gradcheck(f, v, atol=0.2), "Finite difference gradients do not match autograd gradients"
...
def test_linearize_against_analytic_ate():
    ...
    D_train = Predictive(benchmark_model, num_samples=N_train, return_sites=["X", "A", "Y"])()
    D_train = {k: v.squeeze(-1) for k, v in D_train.items()}
    D_test = Predictive(benchmark_model, num_samples=N_test, return_sites=["X", "A", "Y"])()
