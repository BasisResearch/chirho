...
class SimpleGuide(torch.nn.Module):
    def __init__(self):
        super().__init__()
        self.a_loc = torch.nn.Parameter(torch.tensor(0.0))
        self.b_loc = torch.nn.Parameter(torch.tensor(0.0))
...
def test_nmc_log_likelihood():
    ...
    data = Predictive(
        model, guide=guide, num_samples=num_monte_carlo_outer, return_sites=["y"]
    )()
    ...
    nmc_ll_single._vectorized_log_prob({"y": torch.tensor(1.0)})
    nmc_ll({"y": torch.tensor([1.0])})
