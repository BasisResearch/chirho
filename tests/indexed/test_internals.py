...
SHAPE_CASES = list(itertools.product(ENUM_SHAPES, PLATE_SHAPES, BATCH_SHAPES, EVENT_SHAPES))


@pytest.mark.parametrize("enum_shape,plate_shape,batch_shape,event_shape", SHAPE_CASES, ids=str)
def test_indices_of_tensor(enum_shape, plate_shape, batch_shape, event_shape):
    batch_dim_names = {f"b{i}": -1 - i for i in range(len(plate_shape), len(plate_shape) + len(batch_shape))}

    full_batch_shape = enum_shape + batch_shape + plate_shape
    value = torch.randn(full_batch_shape + event_shape)
    actual_world = indices_of(value, event_dim=len(event_shape), name_to_dim=batch_dim_names)
...
def test_indices_of_distribution(enum_shape, plate_shape, batch_shape, event_shape):
    ...
    full_batch_shape = enum_shape + batch_shape + plate_shape
    value = dist.Normal(0, 1).expand(full_batch_shape + event_shape).to_event(len(event_shape))
...
def test_gather_tensor(enum_shape, plate_shape, batch_shape, event_shape, use_effect):
    ...
    world = IndexSet(
        **{name: {full_batch_shape[dim] - 2} for name, dim in name_to_dim.items() if full_batch_shape[dim] > 1}
    )
    ...
    with contextlib.ExitStack() as stack:
        if use_effect:
            ...
            for name, dim in name_to_dim.items():
                add_indices(IndexSet(**{name: set(range(max(2, full_batch_shape[dim])))}))
            _name_to_dim = None
        else:
            _name_to_dim = name_to_dim

        actual = gather(value, world, event_dim=len(event_shape), name_to_dim=_name_to_dim)

    mask = indexset_as_mask(
        world,
        event_dim=len(event_shape),
        name_to_dim_size={name: (dim, full_batch_shape[dim]) for name, dim in name_to_dim.items()},
    )
...
def test_scatter_tensor(enum_shape, plate_shape, batch_shape, event_shape, use_effect):
    ...
    full_batch_shape = enum_shape + batch_shape + plate_shape
    value = torch.randn(enum_shape + (1,) * len(batch_shape) + plate_shape + event_shape)
...
@pytest.mark.parametrize("enum_shape,plate_shape,batch_shape,event_shape", SHAPE_CASES, ids=str)
@pytest.mark.parametrize("use_effect", [True, False])
def test_scatter_gather_tensor(enum_shape, plate_shape, batch_shape, event_shape, use_effect):
    cf_dim = -1 - len(plate_shape)
    name_to_dim = {f"dim_{i}": cf_dim - i for i in range(len(batch_shape))}
...
@pytest.mark.parametrize("enum_shape,plate_shape,batch_shape,event_shape", SHAPE_CASES, ids=str)
@pytest.mark.parametrize("use_effect", [True, False])
def test_gather_scatter_tensor(enum_shape, plate_shape, batch_shape, event_shape, use_effect):
    cf_dim = -1 - len(plate_shape)
    name_to_dim = {f"dim_{i}": cf_dim - i for i in range(len(batch_shape))}
    ...
    world = IndexSet(
        **{name: {full_batch_shape[dim] - 1} for name, dim in name_to_dim.items() if full_batch_shape[dim] > 1}
    )
    ...
    with contextlib.ExitStack() as stack:
        ...
        actual = gather(actual, world, event_dim=len(event_shape), name_to_dim=_name_to_dim)
...
def test_persistent_index_state(enum_shape, plate_shape, batch_shape, event_shape):
    ...
    with IndexPlatesMessenger(cf_dim) as index_state:
        for name, dim in name_to_dim.items():
            add_indices(IndexSet(**{name: set(range(max(2, (batch_shape + plate_shape)[dim])))}))

    with index_state:
        actual = scatter_n({ind1: value1, ind2: value2}, result=result, event_dim=event_dim)
    ...
    with index_state:
        ...
        assert indices_of(actual, event_dim=event_dim) == union(ind1, ind2, indices_of(actual1, event_dim=event_dim))
...
def test_cond_tensor_associate(enum_shape, batch_shape, plate_shape, event_shape):
    ...
    value2 = torch.randn(enum_shape + batch_shape + (1,) * len(plate_shape) + event_shape)
