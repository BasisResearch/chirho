...
import chirho.interventional.handlers
from chirho.counterfactual.handlers import (  # TwinWorldCounterfactual,
    MultiWorldCounterfactual, SingleWorldCounterfactual, SingleWorldFactual,
...
def test_undo_split_with_interaction():
    def model():
        ...
        x = pyro.deterministic("x_split", split(x, (torch.tensor(0.0),), name="x_split", event_dim=0), event_dim=0,)

        x = pyro.deterministic("x_undone", undo_split(antecedents=["x_split"])(x), event_dim=0)

        x_case = torch.tensor(1)
        x = pyro.deterministic("x_preempted",preempt(x, (torch.tensor(5.0),), x_case, name="x_preempted", event_dim=0),
            event_dim=0,)

        x = pyro.deterministic("x_undone_2", undo_split(antecedents=["x"])(x), event_dim=0)

        x = pyro.deterministic("x_split2",split(x, (torch.tensor(2.0),), name="x_split2", event_dim=0),
            event_dim=0,)
    ...
    with mwc:
        assert (
            nd["x_split"]["value"][0].item() == 1.0 and
            nd["x_split"]["value"][1].item() == 0.0 and
            nd["x_undone"]["value"][0].item() == 1.0 and
            nd["x_undone"]["value"][1].item() == 1.0 and
            nd["x_preempted"]["value"][0].item() == 5.0 and
            nd["x_preempted"]["value"][1].item() == 5.0 and
            nd["x_undone_2"]["value"][0].item() == 5.0 and
